    ...
    static class Command
    {
        ...
        public static class Database
        {
            ...
            public static string Analytics(long chat_id)
            {
                ...
                var height = db.Biometries.Where(b => b.Author == chat_id && b.Height != null).OrderBy(x=>x.UpdatedAt).First().Height;
                var weight = db.Biometries.Where(b => b.Author == chat_id && b.Weight != null).OrderBy(x=>x.UpdatedAt).First().Weight;
                var bmi = (double)(height/weight);
                switch(bmi)
                {
                    case < 16:
